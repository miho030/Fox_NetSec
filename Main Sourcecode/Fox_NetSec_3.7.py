...
you can contact me, use gmail => miho0_0@naver.com
...
"""
Main logo printing start
"""
f = Figlet(font='slant')
print(f.renderText('Fox_NetSec'))

print ("              miho0_0@naver.com  한국인터넷진흥원 KUCIS-씨호크    ")
print ("")
print ("")
"""
Main logo printing end
"""
...
# if sysctl.conf file not exist on system, shutdown the program.
def File_reader():
    if os.path.isfile("/etc/sysctl.conf"):
        print("[!] ", "sysctl.conf file is detected! Start the secure modules...\n")
    else:
        print("--------------------------------------------------------------------")
        print("[!] sysctl file is not exist! check your '/etc/sysctl.conf' file !")
        print("")
        print("     [-] I think your system looks not a linux system. X( ")
        print("--------------------------------------------------------------------\n\n")
        sys.exit()


    """
    If your system is linux, this code will be started.
    find sysctl.conf file and open it, and here we go!
    """
    ...
    filedata.write(
        "#  Fox_NetSec secure and check security leel about Linux/Unix server system's sysctl.conf file. \n\n\n")
